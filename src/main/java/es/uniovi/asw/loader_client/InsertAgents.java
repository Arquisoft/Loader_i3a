...
public class InsertAgents {
	...
	for(Agent agent : agentsToInsert) {
	    ...
	    try {
		HttpResponse<JsonNode> jsonResponse = Unirest.post("http://asw-i3a.guill.io:9001/agents_service/register").header("Content-Type", "application/json").
		    body(new JSONObject(agentData)).asJson();
